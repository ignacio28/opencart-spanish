<?php
// Heading
$_['heading_title']   = 'Mi lista de deseos';

// Text
$_['text_account']    = 'Cuenta';
$_['text_instock']    = 'Disponible';
$_['text_wishlist']   = 'Lista de deseos (%s)';
$_['text_login']      = 'Necesitas <a href="%s">iniciar sesión</a> o <a href="%s">crear una cuenta</a> para guardar <a href="%s">%s</a> en tu <a href="%s">lista de deseos</a>!';
$_['text_success']    = 'Éxito: ¡Has añadido <a href="%s">%s</a> a tu <a href="%s">lista de deseos</a>!';
$_['text_remove']     = 'Éxito: ¡Has modificado tu lista de deseos!';
$_['text_no_results'] = 'Tu lista de deseos está vacía.';

// Column
$_['column_image']    = 'Imagen';
$_['column_name']     = 'Nombre del producto';
$_['column_model']    = 'Modelo';
$_['column_stock']    = 'Stock';
$_['column_price']    = 'Precio por unidad';
$_['column_action']   = 'Acción';